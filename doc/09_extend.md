...
首先定义一个 `Axios` 类型接口，它描述了 `Axios` 类中的公共方法，接着定义了 `AxiosInstance` 接口继承 `Axios`，它就是一个混合类型的接口，本身是一个函数（参数为config,函数返回promise),又继承了axios的属性方法。

另外 `AxiosRequestConfig` 类型接口中的 `url` 属性变成了可选属性。buidurl中url推断可能为undefined,但执行buildurl必须有url，可以用类型断言URL！保证存在
